...
/** Return the latest generation for one unchanged interaction. A canceled latest
 * generation is authoritative and cannot resurrect an older completed result. */
export function latestVisualRequestForInteraction(root: string, dir: string, interaction: Interaction | CanonicalInteraction): VisualProtocolRequest | null {
  const canonical = canonicalInteractionJson(interaction);
  const requests = requestIdsForTicket(root, dir)
    .map((requestId) => readVisualRequest(root, dir, requestId))
    .filter((request): request is VisualProtocolRequest => request !== null && request.interactionId === interaction.id && canonicalInteractionJson(request.interaction) === canonical)
    .sort((left, right) => left.requestedAt.localeCompare(right.requestedAt) || left.requestId.localeCompare(right.requestId));
  const latest = requests.at(-1);
  return latest !== undefined && latest.state !== 'canceled' ? latest : null;
}
