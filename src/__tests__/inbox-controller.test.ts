...
pureNotificationController.handleKey('', key({ backTab: true }));
assert.ok(!pureNotificationController.render().join('\n').includes('follow-up'), 'a pure notification neither receives initial follow-up state nor advertises the action');
pureNotificationController.handleKey('?', key());
assert.equal(readFollowUp(pureNotification.dir).request?.requestId, 'notification-request', 'a pure notification cannot dispatch a follow-up callback');
...
pureNotificationController.handleKey('?', key());
for (const char of 'Explain the choice') pureNotificationController.handleKey(char, key());
pureNotificationController.handleKey('', key({ return: true }));
...
assert.ok(mixedFollowUpController.render().join('\n').includes('follow-up'), 'a mixed deck remains follow-up eligible when it contains an answer-bearing interaction');
mixedFollowUpController.handleKey('?', key());
for (const char of 'Explain the choice') mixedFollowUpController.handleKey(char, key());
mixedFollowUpController.handleKey('', key({ return: true }));
