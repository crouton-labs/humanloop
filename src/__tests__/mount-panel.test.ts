...
assert.ok(pendingFrame.split('\n').some((line) => line.includes('▸') && line.includes('Reject')), 'a pending follow-up keeps the option-focus cursor, because j/k and shortcuts stay live');
followUpPanel.setFollowUpState({ status: 'idle' });
const restoredFrame = followUpPanel.render().join('\n').replace(/\x1b\[[0-9;]*m/g, '');
assert.ok(restoredFrame.split('\n').some((line) => line.includes('▸') && line.includes('Reject')), 'focus stays at the retained action index when pending clears');
sendFollowUpKey('q');
sendFollowUpKey('q');
